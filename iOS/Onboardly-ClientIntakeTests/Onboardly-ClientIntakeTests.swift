import XCTest
@testable import Onboardly-ClientIntake

@MainActor
final class Onboardly-ClientIntakeTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        let item = ClientTask(title: "Test Item")
    ...
    func testAddRespectsFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(ClientTask(title: "Item \(i)"), isPro: false)
        }
        let added = store.add(ClientTask(title: "Overflow"), isPro: false)
    ...
    func testProBypassesFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(ClientTask(title: "Item \(i)"), isPro: true)
        }
        let added = store.add(ClientTask(title: "Extra"), isPro: true)
        XCTAssertTrue(added)
        XCTAssertEqual(store.items.count, Store.freeLimit + 1)
    }

    func testDeleteByIdRemovesItem() {
        let item = ClientTask(title: "Delete Me")
        _ = store.add(item, isPro: false)
    ...
    func testUpdateChangesTitle() {
        var item = ClientTask(title: "Original")
        _ = store.add(item, isPro: false)
        item.title = "Updated"
        store.update(item)
    ...
    func testPersistenceRoundTrip() {
        let item = ClientTask(title: "Persisted")
        _ = store.add(item, isPro: false)
        let reloaded = Store()
